    ...
    constructor(query, objectFilter) {
        this.getItems = (pathRequest) => {
            ...
            if (pathRequest.fullPath === '') {
                ...
                var table = findFirstItemMatchesCondition(query().tables(), table => table.actualName() === pathRequest.fullPath);
        ...
        this.getColumnInfo = (propertyName) => {
            if (propertyName) {
                var table = find(query().tables(), t => propertyName.indexOf(t.actualName() + '.') === 0);
                if (table) {
                    var column = find(objectFilter.filterColumns(table.columns() || []), c => propertyName === (table.actualName() + '.' + objectFilter.getColumnName(c)));
