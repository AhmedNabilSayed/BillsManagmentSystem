...
import { appendStaticContextToRootViewModel, _wrapModelInObservable } from '../core/utils/_utils';
...
function customizeDesignerActions(designerModel, nextCustomizer) {
    ...
    return ((actions) => {
        var del = findFirstItemMatchesCondition(actions, action => action.text === 'Delete');
        del.imageClassName = 'dx-icon-dxrd-image-recycle-bin';
        del.imageTemplateName = 'dxrd-svg-operations-recycle_bin_xl';
        var undo = findFirstItemMatchesCondition(actions, action => action.text === 'Undo');
        undo.disabled = ko.pureComputed(() => designerModel.isLoading() || !designerModel.undoEngine().undoEnabled());
        var redo = findFirstItemMatchesCondition(actions, action => action.text === 'Redo');
...
function _createDesignerModel(query, surface, options) {
    ...
    var selection = new SurfaceSelection(['alias', 'name', 'sortOrder']);
    var designer = createDesigner(query, surface, controlsFactory, undefined, undefined, undefined, !!options.rtl, selection);
    designer.addDisposables(querySubscription);
    designer.findControl = (s, e) => {
        var $childs = $.fn.constructor('.dxqb-main').children('.dxrd-control');
        $childs.each((_, child) => {
...
function _updateSurfaceContentSizeSubscription(element, designerModel, surface, callback) {
    var updateSurfaceContentSize_ = updateQueryBuilderSurfaceContentSize(() => $.fn.constructor(element).find('.dxqb-designer'), designerModel.surfaceSize, surface);
...
function _createQueryBuilder(element, options, callbacks, applyBindings = true) {
    ...
    var previewPopupContainer = getParentContainer;
    ...
    var tablesTop = ko.observable(calculateWithZoomFactor(355)), itemPropertiesTabInfoModel = {
    ...
    var tabPanelItem = new AccordionTabInfo(query, itemPropertiesTabInfoModel, designerModel.undoEngine, designerModel.selection.focused, parametersMode === ParametersMode.ReadWrite);
...
export function createQueryBuilder(element, options, callbacks, applyBindings = true) {
    ...
    return resolveFromPromises(promises, () => _createQueryBuilder(element, options, callbacks, applyBindings));
}
export function createQueryBuilderSurface(element, options, queryCreator) {
    var query = ko.observable(), surface = ko.observable();
    registerControls();
    var designerModel = _createDesignerModel(query, surface, options);
    var selectedTabPanel = new SelectedTabInfo(new ObjectProperties(designerModel.editableObject, null, 1));
    var tabPanel = designerModel.tabPanel;
    var switcher = new RightPanelSwitcher(tabPanel.collapsed, designerModel.editableObject, options.showPropertyGridCondition, AccordionTabInfo._getSelectedItemPropertyName);
