...
export class DataSourceParameter extends Disposable {
    ...
    static _getTypeValue(typeName, resultType = null) {
        var result = DataSourceParameter._typeValues.filter((type) => { return type.name === typeName; });
        if (result.length > 0) {
    ...
    get specifics() {
        var realTypeName = (IsDataAccessExpression(this.type.peek()))
            ? this.resultType()
            : this.type();
        var result = DataSourceParameter._typeValues.filter((type) => { return type.name === realTypeName; });
        if (result.length > 0)
