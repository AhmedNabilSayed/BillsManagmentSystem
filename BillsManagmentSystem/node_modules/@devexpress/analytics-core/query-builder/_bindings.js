...
import { convertFromCssPixelUnits, convertToCssPixelUnits } from '../core/internal/_utils';
ko.bindingHandlers['dxdTableView'] = {
    init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
        var templateHtml = getTemplate('dxd-tableview'), $element = $.fn.constructor(element).append(templateHtml), $titles, $content, rtl = !!bindingContext.$root.rtl, value = ko.unwrap(valueAccessor());
        var onDataScroll = (e) => {
        ...
        $element.find('.dxd-tableview-titles .dxd-tableview-resizable').each((index, resizable) => {
            var $title = $.fn.constructor(resizable).find('.dxd-tableview-cell-text');
            var $column = $element.find('.dxd-tableview-data .dxd-tableview-resizable' + index);
            if (index < value.schema.length - 1) {
                var resizableElement = new Resizable(resizable, {
                    handles: 'e',
                    start: (e, ui) => { },
                    stop: () => { },
                    resize: (e, element) => {
