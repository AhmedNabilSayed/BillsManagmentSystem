        ...
        this.getItems = (pathRequest) => {
            ...
            if (!pathRequest.fullPath) {
                ...
                dbSchemaProvider.getDbStoredProcedures().done((storedProcedures) => {
                    if (this._procedures.children().length === 0) {
                        var procedures = storedProcedures.map(proc => {
                            var node = new TreeLeafNode(proc.name, StoredProceduresQueryControl.generateStoredProcedureDisplayName(proc), 'procedure', this._procedures.checked.peek(), proc.arguments, afterCheckToggled);
                ...
                var queries = customQueries().map(query => {
                    ...
                    if (currentQuery)
                        return currentQuery;
                    var queryNode = new TreeQueryNode(name, name, 'query', !!currentQuery && currentQuery.checked(), query.parameters, this._callBack, afterCheckToggled, query);
