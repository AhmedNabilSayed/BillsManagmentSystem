            ...
            if (pathRequest.fullPath === 'queries') {
                var queries = customQueries().map(query => {
                    ...
                    if (query instanceof UnionQuery) {
                        var callback = ko.observable(extend(this._callBack(), { showQbCallBack: callBacks.unionCallBack }));
                    }
                    else if (query instanceof TransformQuery) {
                        var callback = ko.observable(extend(this._callBack(), { showQbCallBack: callBacks.transformCallBack }));
                    }
                    else {
                        var callback = ko.observable(extend(this._callBack(), { showQbCallBack: callBacks.joinCallBack }));
                    }
                    var queryNode = new TreeQueryNode(name, name, 'query', !!currentQuery && currentQuery.checked(), ko.observableArray([]), callback, afterCheckToggled, query);
