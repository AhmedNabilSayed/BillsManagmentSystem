...
export class FederationSelectQueryBuilderPopup extends FederationQueryBuilderPopupBase {
    constructor(onSaveCallback, dataSource, rtl = false, onCloseCallback) {
        ...
        var cellTemplateMask = (container, options) => {
            ...
            var templateHtml = getTemplate('dxrd-querybuilder-column-combobox-masked'), $element = $.fn.constructor(container).append(templateHtml);
            var model = {
        ...
        var cellTemplate = (container, options) => {
            var $container = $.fn.constructor(container);
                    ...
                    validationRules: [this._aliasValidationRule, {
                            ...
                            validationCallback: (options) => {
                                if (options.value === '' && options.data && options.data.key) {
                                    var existedColumn = ColumnExpressionCollectionHelper.findByName(this._getQuery().columns, options.data.key);
    ...
    _onContentReady(e) {
        if (!e.component._isFirstLoad) {
            ...
            var element = $.fn.constructor(e.component.content())[0];
    ...
    _changeColumn(columnData, insertIndex) {
        if (columnData) {
            var query = ColumnExpressionCollectionHelper.createNew(this._getQuery(), this._getQuery().columns, columnData.table, columnData.column);
    ...
    addDataMember(item, position) {
        ...
        var newControl = query.createChild(extend({
            '@ControlType': 'FTable',
            '@Name': name
        }, controlsFactory.controlsMap['FTable'].defaultVal), undefined, item.path);
        var controlSurface = ko.unwrap(findSurface(newControl));
        if (!position) {
            ...
            var _querySurface = ko.unwrap(findSurface(this.designer()));
