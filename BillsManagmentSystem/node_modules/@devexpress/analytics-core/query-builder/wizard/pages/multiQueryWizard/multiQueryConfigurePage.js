...
import { TreeListSearchOptions } from '../../../../widgets/treelist/_treeListSearchViewModel';
...
export class MultiQueryConfigurePage extends WizardPageBase {
    constructor(_options) {
        ...
        this._showQbCallBack = (name = null, isCustomQuery = false) => {
            if (name !== null) {
                var query = findFirstItemMatchesCondition(this._customQueries(), item => name === (item.name() || item.generateName()));
                ...
                if (isCustomQuery) {
                    ...
                    var queryNew = new TableQuery({ '@Name': null }, this._dataSource());
    ...
    _addQueryFromStoredProcedures(elements, dataSource) {
        for (var i = 0; i < elements.children().length; i++) {
            ...
            if (procedure.checked()) {
                var newQuery = new StoredProcQuery({ '@Name': procedure.name, 'ProcName': procedure.name }, dataSource);
    ...
    _addQueryFromCustomQueries(elements, queries, allQueries) {
        for (var i = 0; i < elements.children().length; i++) {
            ...
            var query = findFirstItemMatchesCondition(queries.peek(), item => queryNode.name === (item.name() || item.generateName()));
    ...
    commit() {
        ...
        var customQueries = this._sqlDataSourceWrapper.sqlDataSource.queries().filter(x => checkedCustomQueries.some(queryName => queryName === x.name()) || x instanceof StoredProcQuery);
    ...
    initialize(state) {
        ...
        customQueriesPromise.done((queries) => {
            ...
            this._selectedPath('');
            var itemsProvider = new DBSchemaItemsProvider(this._dataSource().dbSchemaProvider, this._customQueries, this._showQbCallBack, this.disableCustomSql, this._getItemsAfterCheck);
            this._disposables.push(itemsProvider);
            this._itemsProvider(itemsProvider);
            this._getItemsPromise(new PathRequest('queries'));
            var searchOptions = new TreeListSearchOptions();
