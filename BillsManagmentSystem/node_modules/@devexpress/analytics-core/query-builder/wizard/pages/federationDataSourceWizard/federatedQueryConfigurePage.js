...
export class FederatedQueryConfigurePage extends WizardPageBase {
    constructor(_options) {
        ...
        this._setQueryChecked = (query) => {
            ...
            var queries = findFirstItemMatchesCondition(rootItems, (data) => data.name === 'queries');
        ...
        this._getItemsAfterCheck = (node) => {
            ...
            if (node.specifics == 'query') {
                var query = node['query'] || findFirstItemMatchesCondition(this._customQueries(), (data) => (data.alias() || query.generateName()) === node.name);
    ...
    _wrapFieldListCallback(itemsCallback, useCache) {
        return (pathRequest) => {
            var _dataSource = findFirstItemMatchesCondition(this._dataSources(), x => {
    ...
    initialize(state) {
        ...
        var rootItems = ko.observableArray($.map(this._dataSources(), (item) => {
            return {
        ...
        var itemsProvider = new FederationTreeNodeProvider(this._fieldListProvider, rootItems, this._queriesPopupHelper.callBacks, this._customQueries, this._getItemsAfterCheck);
