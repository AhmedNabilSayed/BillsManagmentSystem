...
export class QueryViewModelBase extends QueryElementBaseViewModel {
    ...
    cerateJoinCondition(parentColumn, nestedColumn) {
        ...
        var relation = findFirstItemMatchesCondition(this.relations(), (relation) => {
        ...
        var joinCondition = findFirstItemMatchesCondition(relation.conditions(), (condition) => {
...
export class QueryViewModel extends QueryViewModelBase {
    constructor(querySource, dbSchemaProvider, parametersMode = ParametersMode.ReadWrite, beforeSaveCallback, serializer) {
        ...
        var parameters = deserializeArray(querySource['Parameters'], item => new ParameterViewModel(item, serializer));
        ...
        this.dbSchemaProvider.getDbSchema().done((dbSchema) => {
            ...
            if (dbSourceTable) {
                dbSourceTable.foreignKeys.forEach((fk) => {
                    var pkTable = getFirstItemByPropertyValue(this.tables.peek(), 'name', fk.primaryKeyTable);
                    if (pkTable) {
                        for (var i = 0; i < fk.primaryKeyColumns.length; i++) {
                            var column1 = getFirstItemByPropertyValue(sourceTable.columns(), 'name', fk.columns[i]);
                            var column2 = getFirstItemByPropertyValue(pkTable.columns(), 'name', fk.primaryKeyColumns[i]);
                            if (column1 && column2) {
                ...
                if (dbTable) {
                    dbTable.foreignKeys.forEach((fk) => {
                        if (fk.primaryKeyTable === sourceTable.name()) {
                            for (var i = 0; i < fk.primaryKeyColumns.length; i++) {
                                var column1 = getFirstItemByPropertyValue(sourceTable.columns(), 'name', fk.primaryKeyColumns[i]);
                                var column2 = getFirstItemByPropertyValue(table.columns(), 'name', fk.columns[i]);
                                if (column1 && column2) {
                                    this.cerateJoinCondition(column2, column1);
                                }
                            }
