...
export function createFunctionCompletion(fnInfo, name, insertValue = null) {
    var _insertValue = insertValue || fnInfo.text;
    return {
        caption: name,
        snippet: _insertValue,
        ...
        completer: {
            insertMatch: (editor, data) => {
                ...
                if (_insertValue.substr(0, 3) === '[].') {
                    var ranges = editor.selection.getAllRanges();
                    for (var i = 0, range; range = ranges[i]; i++) {
                        range.start.column -= 2;
                        if (editor.session.getTextRange(range) === '].')
                            _insertValue = _insertValue.substr(3);
                    }
                }
                editor.insert(_insertValue);
                setCursorInFunctionParameter(fnInfo.paramCount, editor, _insertValue);
            }
        }
    };
}
