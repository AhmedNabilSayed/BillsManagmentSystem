...
import { addDisposeCallback, roundingXDecimals } from '@devexpress/analytics-core/analytics-internal';
import * as ko from 'knockout';
import * as $ from 'jquery';
ko.bindingHandlers['toView'] = {
    init: (element, valueAccessor, allBindings, viewModel, bindingContext) => {
        var $previewPage = $.fn.constructor(element), $container = $previewPage.parent('.dxrd-report-preview-holder'), pageActive = valueAccessor().active, subscription = pageActive.subscribe((active) => {
            if (active) {
...
ko.bindingHandlers['lazyImages'] = {
    init: (element, valueAccessor, allBindings, viewModel, bindingContext) => {
        var $element = $.fn.constructor(element), enabled = valueAccessor().enabled, load = null, loadVisibleImages = (time = 300) => {
    ...
    init: (element, valueAccessor, allBindings, viewModel, bindingContext) => {
        var keyDownHandler = function (e) {
            ...
            if (!value || !(e.ctrlKey || e.metaKey)
                || $.fn.constructor(e.target).is('input:visible,textarea:visible')
                || window.getSelection && window.getSelection() && window.getSelection().toString()
                || document['selection'] && document['selection'].createRange().text) {
                return;
            }
            var $clipboardContainer = $.fn.constructor('#clipboard-container');
            $clipboardContainer.empty().show();
            $.fn.constructor("<textarea id='clipboard'></textarea>").val(value)
                .appendTo($clipboardContainer)
                .focus()
                .select();
        };
        var keyUpHandler = function (e) {
            if ($.fn.constructor(e.target).is('#clipboard')) {
                $.fn.constructor('#clipboard-container').empty().hide();
            }
        };
        ...
        var updateZoom = (newOptions) => {
            ...
            if (autoFitBy != ZoomAutoBy.None) {
                if (options.skipIfInvisible && $element.filter(':visible').length == 0)
                    return;
                var newZoom = roundingXDecimals(updatePreviewZoomWithAutoFit(newOptions.width(), newOptions.height(), $element, autoFitBy), true);
...
ko.bindingHandlers['childStyle'] = {
    init: function (element, valueAccessor) {
        ...
        $.fn.constructor(element).find(values.selector).css(values.style);
    }
};
