...
*/
import { ajax, formatUnicorn, getErrorMessage } from '@devexpress/analytics-core/analytics-internal';
...
export class PreviewRequestWrapper {
    ...
    static getProcessErrorCallback(reportPreview, defaultErrorMessage, showMessage = true) {
        return function (message, jqXHR, textStatus) {
            if (jqXHR) {
                var statusCodeText = getLocalization(jqXHR.statusText, 'ASPxReportsStringId.HttpResponseStatusCode_' + jqXHR.status);
                if (defaultErrorMessage) {
                    var stringFormat = getLocalization('{0} ({1} {2})', 'ASPxReportsStringId.Error_WithStatus');
                    var messageWithStatusCode = formatUnicorn(stringFormat, defaultErrorMessage, jqXHR.status, statusCodeText);
                }
                else {
    ...
    static getPage(url, ignoreError) {
        return ajax(url, undefined, undefined, PreviewRequestWrapper.getProcessErrorCallback(), ignoreError, { type: 'GET' });
    }
    ...
    findTextRequest(text, ignore) {
        return ajax(HandlerUri(), 'findText', encodeURIComponent(JSON.stringify({
            text: text,
    ...
    stopBuild(id) {
        ajax(HandlerUri(), 'stopBuild', encodeURIComponent(id), undefined, () => true);
    }
    sendCloseRequest(documentId, reportId) {
        ajax(HandlerUri(), 'close', encodeURIComponent(JSON.stringify({
            reportId: reportId,
    ...
    getDocumentData(documentId, shouldIgnoreError) {
        return ajax(HandlerUri(), 'getDocumentData', encodeURIComponent(documentId), PreviewRequestWrapper.getProcessErrorCallback(this._reportPreview, getLocalization('Cannot obtain additional document data for the current document', 'ASPxReportsStringId.WebDocumentViewer_GetDocumentDataError')), shouldIgnoreError);
    }
    customDocumentOperation(documentId, serializedExportOptions, editindFields, customData, hideMessageFromUser) {
        return ajax(HandlerUri(), 'documentOperation', encodeURIComponent(JSON.stringify({
            documentId: documentId,
    ...
    openReport(reportName) {
        return ajax(HandlerUri(), 'openReport', encodeURIComponent(reportName), PreviewRequestWrapper.getProcessErrorCallback(this._reportPreview, getLocalization('Could not open report', 'ASPxReportsStringId.WebDocumentViewer_OpenReportError') + " '" + reportName + "'"));
    }
    drillThrough(customData) {
        return ajax(HandlerUri(), 'drillThrough', encodeURIComponent(JSON.stringify({
            reportId: this._reportPreview.reportId,
            reportUrl: this._reportPreview.reportUrl,
            documentId: this._reportPreview.documentId,
    ...
    getStartExportOperation(arg, shouldIgnoreError) {
        return ajax(HandlerUri(), 'startExport', arg, PreviewRequestWrapper.getProcessErrorCallback(this._reportPreview, getLocalization('An error occurred during the export', 'ASPxReportsStringId.WebDocumentViewer_ExportError')), shouldIgnoreError);
    }
    cancelExportRequest(operationId, shouldIgnoreError) {
        var arg = encodeURIComponent(JSON.stringify({
            id: operationId
        }));
        return ajax(HandlerUri(), 'cancelExport', arg, PreviewRequestWrapper.getProcessErrorCallback(this._reportPreview, getLocalization('An error occurred during the export', 'ASPxReportsStringId.WebDocumentViewer_ExportError')), shouldIgnoreError);
    }
    ...
    getEditingFieldHtml(value, editingFieldIndex) {
        return ajax(HandlerUri(), 'getEditingFieldHtmlValue', encodeURIComponent(JSON.stringify({
            documentId: this._reportPreview.documentId,
            value: value,
