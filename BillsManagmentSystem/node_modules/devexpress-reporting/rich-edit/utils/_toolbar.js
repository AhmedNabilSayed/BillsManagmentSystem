...
export class ComponentButtonGroup extends ComponentCommon {
    ...
    updateState() {
        ...
        this.items.forEach(item => {
            var command = this.getCommand(item);
            if (item.command === command) {
                var commandState = this.options.richEditPublic.getCommandState(command);
...
export class ToolbarSurface extends Disposable {
    constructor(options) {
        ...
        var toolbarItems = extend(true, [], this._getDefaultItems(options.fonts));
        var getById = (itemId) => {
            var matchedItem;
            var group = toolbarItems.filter(item => {
                if (matchedItem)
                    return false;
                if (item.id === itemId)
                    return true;
    ...
    onCommandStateChanged(sender, args) {
        if (args.commands) {
            ...
            for (var group of this.componentCollection) {
                if (group.items) {
