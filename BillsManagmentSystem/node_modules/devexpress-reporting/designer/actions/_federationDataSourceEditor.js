...
export class FederationDataSourceEditor extends DataSourceEditorBase {
    constructor() {
        ...
        this.editAction = {
            clickAction: (item) => {
                var dataSourceInfo = this._findDataSource(item.path.split('.')[0]);
                this.getFederationDataSourceByInfo(dataSourceInfo).done(federationDataSource => {
                    var query = federationDataSource.queries().filter(x => x.alias() === item.name)[0];
    ...
    applyFederationDataSourceWizardChanges(dataSourceWizardModel) {
        var federationDataSource = _restoreFederationDataSourceFromState(dataSourceWizardModel.federationDataSourceWizard, this._dsHelper().usedDataSources, dataSourceWizardModel.dataSourceId);
    ...
    getFederationDataSourceByInfo(dataSourceInfo) {
        var deferred = $.Deferred();
        var rootManageQueriesDataSources = ko.observableArray(this._dsHelper().usedDataSources().filter(x => x !== dataSourceInfo));
        ReportDataSourceService.federationDataSourceFromBase64(dataSourceInfo.data['base64'](), dataSourceInfo.data.dependentDataSources).done((result) => {
            var fieldListProvider = new FieldListProvider(this._wrapFieldListCallback(this._callbacks.fieldListsCallback), rootManageQueriesDataSources, [new FederationDataSourceItemsExtender(rootManageQueriesDataSources)], true);
            var federationDataSource = new FederationDataSource(JSON.parse(result.federationDataSourceJSON), rootManageQueriesDataSources, fieldListProvider);
