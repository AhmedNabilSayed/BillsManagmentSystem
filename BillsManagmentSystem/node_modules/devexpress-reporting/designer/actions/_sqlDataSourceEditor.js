...
import { ajax, getErrorMessage, ShowMessage } from '@devexpress/analytics-core/analytics-internal';
...
export class SqlDataSourceEditor extends DataSourceEditorBase {
    ...
    _createOrEditSqlDataSource(requestJson, dataSource, requestName) {
        return ajax(HandlerUri(), requestName, encodeURIComponent(requestJson))
            .done((result) => {
    ...
    editSqlQuery(dataSourceID, queryName) {
        ...
        ReportDataSourceService.sqlDataSourceFromBase64(dataSourceInfo.data['base64']()).done((result) => {
            var sqlDataSource = new SqlDataSource(JSON.parse(result.sqlDataSourceJSON), undefined, QBRequestWrapper());
    ...
    removeSqlQuery(dataSourceID, queryName) {
        var dataSourceInfo = this._findDataSource(dataSourceID);
        ReportDataSourceService.sqlDataSourceFromBase64(dataSourceInfo.data['base64']()).done((result) => {
            var sqlDataSource = new SqlDataSource(JSON.parse(result.sqlDataSourceJSON));
    ...
    applySqlDataSourceWizardChanges(dataSourceWizardModel) {
        var dataSourceWrapped = _restoreSqlDataSourceFromState(dataSourceWizardModel.sqlDataSourceWizard, QBRequestWrapper(), dataSourceWizardModel.dataSourceId);
