...
export class DataBindingBase extends Disposable {
    ...
    _findDataSourceFromPath(path, dataSources) {
        ...
        var dataSourceInfo = (dataSources || []).filter((dataSource) => { return dataSource.ref === dataSourceId || dataSource.id === dataSourceId; })[0];
        if (dataSourceInfo) {
...
export class DataBinding extends DataBindingBase {
    ...
    static initialize(model, serializer) {
        var currentDataBindings = deserializeArray(model, (item) => { return new DataBinding(item, serializer); });
