...
export class Parameter extends Disposable {
    constructor(model, _report, serializer) {
        ...
        this.expressionObj = {
            getInfo: () => {
                var info = this.getInfo().filter(x => x.propertyName.indexOf(ParameterExpressionBinding.expressionSuff) != -1);
                info.filter(x => x.propertyName == 'ValueExpressionObj')[0].displayName = 'Value';
                return info;
            }
        };
        this.info.forEach(property => {
            var index = property.propertyName.indexOf(ParameterExpressionBinding.expressionSuff);
            if (index !== -1) {
        ...
        this._disposables.push(this._isEditing.subscribe((newVal) => {
            var settigns = this.valueSourceSettings();
        ...
        this.valueInfo = ko.pureComputed(() => {
            var result = $.extend(true, {}, parameterValueSerializationInfo, parameterHelper.getParameterInfo(this));
    ...
    _initializeValue() {
        ...
        if (this.isMultiValue()) {
            typeof value === 'string'
                ? this.value = this._parameterHelper.createMultiValueArray(value.split(parameterSeparator), this, (part) => { return this.parameterTypesHelper.convertSingleValue(part, this.type()); })
                : this.value = ko.observableArray();
        }
    ...
    _getExpressionActions(name) {
        if (Parameter.propertiesWithExpressions.indexOf(name) === -1)
            return;
        var propertyInfo = this.info.filter(x => x.propertyName === name)[0];
        var expression = this[this.propertyExpressionMapper.getExpressionPropertyName(name)];
        var expressionLocalizedName = getLocalization(propertyInfo.displayName, propertyInfo.localizationId);
        var expressionForLocalizedString = getLocalization('{0} Expression', 'ReportStringId.UD_PropertyGrid_Menu_ItemExpression');
        var expressionHint = ko.pureComputed(() => { return expression.value(); });
        this._disposables.push(expressionHint);
        if (!this._expressionActions[name]) {
            var expressionEditor = new ReportExpressionEditorWrapper(ko.observable(this), ko.observable(expression));
    ...
    _updateTypeValues() {
        var types = this.info.filter(info => info.propertyName == 'type')[0];
        var knownEnumValues = this.parameterTypesHelper.getEnumTypeValues();
    ...
    getInfo() {
        ...
        if (this.type) {
            ...
            if (Parameter.availableRangeSettingTypes.indexOf(this.type()) !== -1) {
                var parameterSettingsTypeInfo = newInfo.filter((prop) => { return prop.propertyName === 'valueSourceSettingsType'; })[0];
            ...
            if (this.valueSourceSettings() && this.valueSourceSettings() instanceof RangeParametersSettings) {
                var typeInfo = newInfo.filter((prop) => { return prop.propertyName === 'type'; })[0];
                if (typeInfo) {
