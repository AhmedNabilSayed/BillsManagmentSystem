...
export class FormattingRuleEditor extends Editor {
    constructor(info, level, parentDisabled, textToSearch) {
        ...
        var rules = ko.pureComputed(() => {
            var formattingRuleSheet = this._model() && this._model()['root'] && this._model()['root'].formattingRuleSheet || ko.observableArray();
                ...
                this._disposables.push(rule.selected = ko.pureComputed({
                    ...
                    write: (val) => {
                        if (val) {
                            ...
                            var link = this.value()().filter((itemLink) => { return itemLink.value() === rule; })[0];
        ...
        this.options = {
            addHandler: () => { return FormattingRule.createNew(this._model()['root']); },
            values: rules,
