        ...
        this._disposables.push(this.height = ko.pureComputed({
            read: () => {
                if (checkModelReady(this.root)) {
                    var verticalBand = this.bands().filter(x => x instanceof VerticalBandViewModel)[0];
                    var height = 0;
                    if (verticalBand)
            ...
            write: (newHeight) => {
                if (checkModelReady(this.root)) {
                    var deltaHeight = newHeight - this.height.peek(), oldHeight = this.bands()[this.bands().length - 1].height.peek();
...
export class DetailReportBandSurface extends BandSurface {
    constructor(band, context) {
        ...
        this._disposables.push(ko.computed(() => {
            var isSomeParentCollapsed = this.collapsed() || this.isSomeParentCollapsed();
    ...
    createUnderCursor() {
        var _underCursor = ko.observable(new HoverInfo());
