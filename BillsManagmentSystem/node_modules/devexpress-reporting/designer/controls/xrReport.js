...
export class ReportViewModel extends XRReportElementViewModel {
    constructor(report, serializer, knownEnums) {
        ...
        this.styles = deserializeArray(report.StyleSheet, (item) => { return new StyleModel(item, serializer); });
        ...
        var dataMember = ko.pureComputed(() => {
            return getFullPath(this.getPath('dataMember'), this.dataMember());
        });
        var dataSource = this.dataSource;
        var disabled = ko.pureComputed(() => !dataSource());
        var filterString = new FilterStringOptions(this['_filterString'], dataMember, disabled);
        ...
        this.watermark = new WatermarkModel(report['Watermark'], serializer);
        ...
        var currentLanguage = defaultCulture;
    ...
    static createObjectStorage(_componentStorage, _objectStorage, collectSubscription = (subscription) => void 0) {
        var objectStorage = ko.observableArray([
            ..._objectStorage(),
            ..._componentStorage()
        ]);
    ...
    _updatePageSize(paperKind) {
        var size = paperKind === 'Custom' ? this.size : getPaperSize(paperKind);
    ...
    getOrCreateBandForToC(createNew = true) {
        ...
        var bands = this.bands().filter(element => {
    ...
    _initializeBands() {
        var traverse = (xs) => xs.reduce((res, x) => { res.push(x, ...traverse(ko.unwrap(x['bands']) || [])); return res; }, []);
        var bandNames = traverse(this.bands()).map(x => x.name());
    ...
    clone(dataSourceRefsFromParent) {
        var filteredDataSourceRefs = [];
        var dataSourceRefs = [...this.dataSourceRefs];
        ...
        var collectStorages = (storage, isComponentStorage = false) => {
            storage.reduce((result, storageItem, index) => {
                var dataSourceRef = dataSourceRefs.filter(x => x.ref === storageItem['_model']['@Ref'])[0];
                if (dataSourceRef) {
                    result.push({
                        index,
                        isComponentStorage,
...
export class ReportSurface extends SurfaceElementArea {
    constructor(report, zoom = ko.observable(1)) {
        ...
        this._disposables.push(report.watermark.imageSource.subscribe((newValue) => {
            this._updateWatermarkImageNaturalSize(newValue);
        }));
        this._updateWatermarkImageNaturalSize(report.watermark.imageSource.peek());
        this._disposables.push(this._watermarkTextRenderingResult = ko.pureComputed(() => {
            var canvas = document.createElement('canvas');
            var originalWidthPx = unitsToPixel(report.pageWidth(), this.measureUnit());
            var originalHeightPx = unitsToPixel(report.pageHeight(), this.measureUnit());
            ...
            var font = new CssCalculator(report.watermark, ko.observable(false)).fontCss();
        ...
        this._disposables.push(ko.computed(() => {
            var levelCount = getLevelCount(this.bandsHolder);
        ...
        this._disposables.push(this.effectiveHeight = ko.pureComputed(() => {
            var minHeight = this.bandsHolder.getHeight();
    ...
    wrapRtlProperty(data, undoEngine, element) {
        var wrapper = ko.computed({
            ...
            write: (newValue) => {
                ...
                var prevValue = this.rtl.peek();
                data.value(newValue);
                if (prevValue !== this.rtl.peek()) {
                    var report = this.getControlModel();
...
class SurfaceMarginResizeOptions extends Disposable {
    constructor(margins, rtl, pageWidth) {
        ...
        this.rightMarginOptions = (undoEngine, element) => {
            ...
            var resizableElement = new Resizable(element, rightOptions).initialize();
        ...
        this.leftMarginOptions = (undoEngine, element) => {
            ...
            var resizableElement = new Resizable(element, leftOptions).initialize();
            addDisposeCallback(element, () => {
                resizableElement.dispose();
            });
            elements.push(element);
