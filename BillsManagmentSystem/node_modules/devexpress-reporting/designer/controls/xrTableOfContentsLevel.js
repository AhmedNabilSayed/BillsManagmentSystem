...
*/
import { ElementViewModel, PaddingModel } from '@devexpress/analytics-core/analytics-elements';
...
export class TableOfContentsLevel extends ElementViewModel {
    constructor(model, parent, serializer, isTitle = false) {
        ...
        this._disposables.push(this.top = ko.computed({
            read: () => {
                var previousLevel = parent.allLevels()[this._levelIndex() - 1];
    ...
    _createPaddingDependencies() {
        ...
        this.paddingObj.dpi = ko.computed(() => this.dpi && this.dpi());
...
export class TableOfContentsLevelSurface extends XRControlSurfaceBase {
    constructor(control, context) {
        ...
        this.leaderSymbols = ko.pureComputed(() => {
            var elementWidth = this.contentWidthWithoutZoom();
            if (elementWidth <= 0 || this._leaderSymbolWidth() === 0)
                return '';
            var symbolsCount = elementWidth / this._leaderSymbolWidth();
            var symbol = this.getControlModel().leaderSymbol();
            var a = [];
