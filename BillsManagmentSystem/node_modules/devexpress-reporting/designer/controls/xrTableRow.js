...
export class XRTableRowViewModel extends XRControlViewModel {
    constructor(control, parent, serializer) {
        ...
        this._disposables.push(this.height = ko.pureComputed({
            read: () => {
                var result = parent.size.height() * this.weight() / parent.rowsTotalWeight();
                return roundingXDecimals(result);
            },
            write: (newHeight) => {
                var dHeight = newHeight - this.height(), newWeight = parent.pixelHeightWeight() * newHeight, dWeight = newWeight - this.weight(), thisRowIndex = parent.rows().indexOf(this);
        ...
        var deserializedCells = deserializeArray(control.Cells, (item) => { return this.getControlFactory().createControl(item, this, serializer); });
        this.cells(deserializedCells());
        var _top = ko.observable(null);
        this._disposables.push(this.top = ko.pureComputed({
            read: () => {
                var index = parent.rows().indexOf(this);
                if (index > 0) {
    ...
    insertCellCopy(selectedCell, isRight, onComponentAdded) {
        var newCellWeight = selectedCell.weight() / 2, newCell = new XRTableCellViewModel({ '@ControlType': 'XRTableCell', '@Weight': newCellWeight, '@Multiline': 'true' }, this), indexSelectedCell = this.cells.indexOf(selectedCell);
