...
export class XRSubreportViewModel extends XRControlViewModel {
    constructor(model, parent, serializer) {
        ...
        var _width = ko.observable(this.size.width());
        this.size['_width'] = _width;
        this._disposables.push(this.key = ko.pureComputed(() => {
            var key = this.parentModel() && this.parentModel().root && this.parentModel().root['key'];
            return (key ? (key() + '.') : '') + (this.name() ? this.name() : '');
        }));
        this._generateOwnPages = ko.observable(this.generateOwnPages());
        var oldgenerateOwnPagesIsActive = this._generateOwnPages();
        this._disposables.push(this.generateOwnPages = ko.pureComputed({
            ...
            write: (newVal) => {
                if (this.isPropertyDisabled('generateOwnPages'))
                    return;
                var undo = UndoEngine.tryGetUndoEngine(this.parentModel());
                undo && undo.start();
        ...
        this.parameterBindings = deserializeArray(model.ParameterBindings, (item) => {
            var binding = new ParameterBinding(item, this, serializer);
    ...
    getInfo() {
        var serializationInfo = $.extend(true, [], super.getInfo());
        if (this.reportSourceUrl && this.reportSourceUrl()) {
            var property = serializationInfo.filter((item) => { return item.propertyName === 'reportSource'; })[0];
    ...
    _calculateSubreportPosition(generateOwnPagesIsActive) {
        var offset = generateOwnPagesIsActive ? 0 : pixelToUnits(5, this.root.measureUnit(), 1);
    ...
    updateParameters() {
        if (this.reportSourceUrl()) {
            ReportStorageWeb.getData(this.reportSourceUrl()).done((result) => {
                ...
                var report = new ReportViewModel(reportJSONModel);
    ...
    cloneReportSource() {
        var _a;
        if (this.reportSource) {
            var dataSourceRefFromParent = (_a = this.root) === null || _a === void 0 ? void 0 : _a.dataSourceRefs;
