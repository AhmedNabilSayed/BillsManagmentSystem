...
export function createReportViewModel(newReportInfo, oldReport) {
    ...
    var report = new ReportViewModel(reportModel, undefined, newReportInfo.knownEnums);
    if (oldReport) {
        var nextRef = Math.max(...report.objectStorage().map(data => parseInt(data['_model']['@Ref']))) + 1;
...
export function updateDataSourceRefs(report, dataSourceRefs) {
    var getDataSourceRefs = (key) => {
        var resultRefs = (dataSourceRefs || []).filter((ds) => { return ds.Key === key; })[0];
...
export function updateSurfaceContentSizeLocalizationMode(surfaceSize, root, rtl) {
    return () => {
        ...
        var leftLocalizationPanel = $.fn.constructor(root).find('.dxrd-left-localization-panel:visible').outerWidth() || 0;
