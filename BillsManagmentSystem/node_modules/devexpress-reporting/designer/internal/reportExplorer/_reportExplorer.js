...
export class ReportExplorerModel extends Disposable {
    constructor(reportModel, editableObject, clickHandler, dragDropHandler, selection, _dataSourceSettings) {
        ...
        this._disposables.push(this.itemsProvider = new ObjectExplorerProvider([
            { model: reportModel, name: 'Report', displayName: getLocalization('Report', 'ASPxReportsStringId.DocumentViewer_RibbonReportGroupText'), className: 'master_report', data: reportModel },
            ...
            { model: ko.pureComputed(() => { return reportModel() && reportModel().crossBandControls(); }), name: 'Crossband Controls', displayName: getLocalization('Cross-Band Controls', 'ASPxReportsStringId.ReportDesigner_CrossBandControls'), className: 'xrcrossbandbox' },
    ...
    _createActionsForOneElement(clickHandler, selection, editableObject, reportModel, item) {
        ...
        if (!isDeleteDeny && !isLocked) {
            actions.push({
                ...
                clickAction: (treeListItemViewModel) => {
                    var selectedObject = treeListItemViewModel === null || treeListItemViewModel === void 0 ? void 0 : treeListItemViewModel.data['data'];
            ...
            return [{
                    ...
                    clickAction: () => {
                        if (item.data.name === 'Styles') {
                            var newStyleName = getUniqueNameForNamedObjectsArray(reportModel().styles(), 'xrControlStyle');
                            reportModel().styles.push(new StyleModel({ '@Name': newStyleName }));
                        }
                        else {
                            reportModel().formattingRuleSheet.push(FormattingRule.createNew(reportModel()));
                        }
                    }
                }];
        }
