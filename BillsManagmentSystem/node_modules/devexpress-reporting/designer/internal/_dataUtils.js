...
export function addDataSourceToReport(dataSourceHelper, report, undoEngine, itemsProvider, dataSource, forceAssigning = false) {
    undoEngine.start();
    var findFirstDataSourceWithSerializer = findFirstItemMatchesCondition(dataSourceHelper.usedDataSources.peek(), item => !!item.dataSerializer && item.dataSerializer !== dataSource.dataSerializer);
    ...
    if (!findFirstDataSourceWithSerializer && dataSource.dataSerializer) {
        ...
        var newDataSerializer = new ExtensionModel({});
    ...
    if (forceAssigning || !report.dataSource()) {
        ...
        itemsProvider
            ...
            .done((dataMembers) => {
            var lists = dataMembers.filter(item => isList(item));
