...
import { memberControlsMap } from './_fieldListDragDropHelper';
...
export class FieldListDragDropHandler extends DragDropHandler {
    ...
    canDrop(dropTarget, controlModel, metaData) {
        var canDrop = super.canDrop(dropTarget, controlModel, metaData);
    ...
    drag(event, ui, draggable) {
        ...
        if (this.selection.dropTarget) {
            var key = this._getKey(draggable);
            var dropTarget = this._getDropTarget(memberControlsMap[key]);
            ...
            if (dropTargetControl['hasDefaultBindingProperty'] && !isList(draggable.data) || dropTarget.dragCallback) {
                if (!this._needToChangeHelperContent(dragHelperContent, boundedClass)) {
                    var rect = new Rectangle(12, 12, 12, 12);
    ...
    doStopDrag(uiElement, draggable, event) {
        ...
        if (this.selection.dropTarget) {
            ...
            if (this.surface().isFit && this.surface().isFit(this.selection.dropTarget) || this.selection.dropTarget.underCursor().isOver) {
                var dropTarget = this._getDropTarget(memberControlsMap[key]), dropTargetControl = dropTarget.getControlModel();
                ...
                if (!isMultiSelect && !isList(item.data)) {
                    var control = (memberControlsMap[key] || memberControlsMap['Default']).drop(item, dropTarget.getControlModel(), this.dataBindingMode());
                    this._addControl(control, dropTarget);
                    return;
                }
                memberControlsMap[isMultiSelect ? 'MultiList' : key]
                    .drop(item, dropTarget.getControlModel(), this.dataBindingMode(), memberControlsMap[isMultiSelect ? 'MultiList' : key].size(this.surface()))
                    .done((control) => {
                    this._addControl(control, dropTarget);
                });
            }
