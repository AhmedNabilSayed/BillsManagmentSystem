...
export class ErrorPanelViewModel extends Disposable {
    ...
    createDataGridOptions(undoEngine) {
        var ds = ko.observable(new DataSource(this._errorList()));
    ...
    _assignFilter() {
        var filter = [];
        var uncheckedTypes = ErrorPanelViewModel.allErrorTypes.map(x => ErrorType[x]).filter(x => this._choosenTypes().indexOf(x) === -1);
        var uncheckedSources = ErrorPanelViewModel.allErrorSources.map(x => ErrorSource[x]).filter(x => this._errorSource().indexOf(x) === -1);
