...
export class CrossTabConverter extends BaseConverter {
    ...
    _applyChanges() {
        ...
        var crossTab = controlsFactory().createControl(controlsFactory().controlsMap['XRCrossTab'].defaultVal, pivotGrid.parentModel());
    ...
    _convertStyles(pivotGrid, crossTab, convertedFields) {
        ...
        crossTab.cells().forEach(cell => {
            ...
            var cellStyle = StylesHelper.generateStyle(deafultStyle);
            ...
            for (var group of resultStyleGroups) {
    ...
    _prepareStandardStyles(resultStyleGroups, crossTab, root) {
        var headerStyle = findFirstItemMatchesCondition(resultStyleGroups, (item) => item.cells.every(x => XRCrossTabCellViewModel.cellKinds.Header.indexOf(x.kind()) !== -1)).style;
        headerStyle.name(crossTab['headerAreaStyleName']());
        if (!headerStyle)
            return false;
        var dataStyle = findFirstItemMatchesCondition(resultStyleGroups, (item) => item.cells.every(x => XRCrossTabCellViewModel.cellKinds.Data.indexOf(x.kind()) !== -1)).style;
        dataStyle.name(crossTab['dataAreaStyleName']());
        if (!dataStyle)
            return false;
        var totalStyle = findFirstItemMatchesCondition(resultStyleGroups, (item) => item.cells.every(x => XRCrossTabCellViewModel.cellKinds.Total.indexOf(x.kind()) !== -1)).style;
        if (!totalStyle)
            return false;
    ...
    _convertFields(pivotGrid, crossTab) {
        var convertedFields = {};
        var addField = (type, pivotField) => {
            var field = crossTab.getInfo().filter(info => info.propertyName === type)[0].addHandler();
            this._copyPropertiesToField(field, pivotField);
            crossTab[type].push(field);
            var dependentCell = crossTab.cells().filter(cell => cell.field() == field)[0];
    ...
    _saveOriginalLayout(pivotGrid, crossTab) {
        ...
        var layout = new ModelSerializer().serialize(pivotGrid);
    ...
    _applyText(pivotGrid, crossTab) {
        crossTab.cells().forEach(cell => {
            ...
            var formatInfo;
            var pivotGridFieldItem = this._findRelatedPivotGridItem(pivotGrid, cell.dataLevel, cell.columnLevel, cell.rowLevel);
            if (cellKind == CellKind.ColumnHeader || cellKind == CellKind.RowHeader) {
                ...
                if (pivotGridFieldItem) {
                    var isTotal = cellKind === CellKind.RowTotal || cellKind === CellKind.ColumnTotal;
                    var isGrandTotal = cellKind === CellKind.GrandTotal;
...
export class PivotGridConverter extends BaseConverter {
    ...
    _applyChanges() {
        ...
        ControlConverterService.getControlModelFromXmlString(model.originalPivotGridLayout(), result => {
            ...
            var pivotGrid = new XRPivotGridViewModel(result, model.parentModel());
