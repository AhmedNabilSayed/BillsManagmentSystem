...
export class ParametersViewModel extends Disposable {
    ...
    afterItemsFilled(request, items) {
        if (!request.fullPath) {
            var parameters = findFirstItemMatchesCondition(items, item => item.specifics === 'parameters');
            if (parameters)
                parameters.displayName = getLocalization('Parameters', 'DevExpress.XtraReports.UI.XtraReport.Parameters');
            var noneDataSource = !request.fullPath && findFirstItemMatchesCondition(items, item => item.specifics === 'none');
            if (noneDataSource)
