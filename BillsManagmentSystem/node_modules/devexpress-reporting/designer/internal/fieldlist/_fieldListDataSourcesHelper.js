...
export var maxNestingLevelUpdate = createGlobalModuleVariableFunc(5);
export function patchRequest(request, dataSources, state) {
    request.state = state;
    var dataSource = findFirstItemMatchesCondition(dataSources, (ds) => (request.id && ds.id === request.id) || (request.ref && ds.ref === request.ref));
...
export class FieldListDataSourcesHelper {
    ...
    _wrapRequest(request) {
        var pathParts = request.pathParts.length > 0 ? request.pathParts.map(x => x) : request.fullPath.split('.');
        var currentPathLength = Math.floor((pathParts.length - 1) / maxNestingLevelUpdate());
        var currentRequestPath = pathParts.splice(0, 1 + currentPathLength * maxNestingLevelUpdate());
    ...
    _findItems(items, pathParts) {
        ...
        var item = items.filter(x => x.name === itemName)[0];
        if (!item)
            return;
    ...
    wrapFieldsCallback(fieldsCallback, state, dataSources = this.fieldListDataSources, useCache = true) {
        ...
        return (request) => {
            if (cache && useCache) {
                var items = this._getItemsFromCache(request);
                if (items)
                    return $.Deferred().resolve(items).promise();
                var newRequest = this._wrapRequest(request);
