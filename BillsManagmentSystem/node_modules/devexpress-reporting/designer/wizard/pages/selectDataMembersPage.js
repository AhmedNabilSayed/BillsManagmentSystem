...
export class SelectDataMembersPage extends WizardPageBase {
    constructor(_fieldListCallBack, _hideDataMemberSubItems = false) {
        ...
        this._createMasterDetailTreeNode = (item, isChecked, path) => {
            var node = new DataMemberTreeNode(item.name, item.displayName, item.specifics, isChecked, path, this._afterCheckToggledFields);
            this._disposables.push(node);
            return node;
        };
        this._createMasterDetailFirstTabTreeNode = (item, isChecked, path) => {
            var checked = isChecked || (this._showDataSource ? [this.dataSourcePath, this.initialFullDataMember].join('.') : this.initialFullDataMember) === path;
            var node = new DataMemberCustomCheckedTreeNode(item.name, item.displayName, item.specifics, checked, path, this._afterCheckToggled);
            this._disposables.push(node);
            return node;
        };
        this._createMasterDetailLeafTreeNode = (item, isChecked, path) => {
            var node = new FieldTreeNode(item.name, item.displayName, item.specifics, isChecked, path, this._afterCheckToggledFields);
            this._disposables.push(node);
            return node;
        };
        ...
        var fieldListProvider = new FieldListProvider(this._wrapFieldListCallback(this._fieldListCallBack), ko.observableArray([]));
        ...
        this._disposables.push(ko.computed(() => {
            var item = this._availableFieldsController.selectedItem;
