...
export class SelectCrossTabDataMember extends SelectDataMembersPage {
    constructor(_fieldListCallBack, _hideDataMemberSubItems = false) {
        ...
        this._createCrossTabLeafTreeNode = (item, isChecked, path) => {
            var field = new FieldTreeNode(item.name, item.displayName, item.specifics, isChecked, path);
        ...
        this._createCrossTabTreeNode = (item, isChecked, path) => {
            var node = new SingleCheckedDataMemberTreeNode(item.name, item.displayName, item.specifics, isChecked, path, this._afteCheck);
        ...
        this._afteCheck = (node) => {
            var rootItems = this._signleFieldMemberFieldListModel.itemsProvider.getRootItems();
        ...
        var fieldListProvider = new FieldListProvider(this._wrapFieldListCallback(_fieldListCallBack), ko.observableArray([]));
    ...
    commit() {
        ...
        var result = dataMember ? {
            crossTabFields: dataMember && dataMember.children().filter(x => !x.isList) || [],
            dataMemberPath: getFullPath(this.dataSourcePath, dataMember.path),
...
export class ConfigureCrossTabPage extends WizardPageBase {
    ...
    addInfo(fieldName) {
        var newField = findFirstItemMatchesCondition(this.fieldInfos(), (item) => item.field() === null);
        if (!newField) {
        ...
        if (fieldName) {
            var field = findFirstItemMatchesCondition(this._crossTabFields(), (item) => item.name === fieldName);
