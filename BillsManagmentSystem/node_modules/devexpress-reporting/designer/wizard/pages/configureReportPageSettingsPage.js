...
export class PreviewPageHelper extends Disposable {
    constructor(settings) {
        ...
        var elementWidth = ko.computed(() => this.pagePreviewElement() && this.pagePreviewElement().width() || 200).extend({ throttle: 100 });
        var elementHeight = ko.computed(() => this.pagePreviewElement() && this.pagePreviewElement().height() || 292).extend({ throttle: 100 });
        var previewRatio = ko.computed(() => Math.min(elementWidth() / this.width(), elementHeight() / this.height()));
        var resizeHandler = () => {
...
export class ConfigureReportPageSettingsPage extends WizardPageBase {
    constructor() {
        ...
        this._disposables.push(this.unit = ko.computed({
            read: () => this._unit(),
            write: (newVal) => {
                var converter = PageSetupHelper.getConverter(this._unit(), newVal);
        ...
        this._disposables.push(this.paperKind.subscribe(newVal => {
            if (newVal !== 'Custom') {
                var convert = PageSetupHelper.getConverter(GraphicsUnit.Inch, this._unit());
                var size = getPaperSize(newVal, 0);
                var width = convert(size.width / 100);
                var height = convert(size.height / 100);
                if (this.landscape())
        ...
        this._disposables.push(this.landscape.subscribe((newVal) => {
            ...
            this.width(width);
            this.height(height);
            var t = this.marginTop(), r = this.marginRight(), b = this.marginBottom(), l = this.marginLeft();
            [t, r, b, l] = newVal
                ? [l, t, r, b]
                : [r, b, l, t];
            this.marginTop(t);
