...
export class LegacyAddGroupingLevelPage extends WizardPageBase {
    ...
    initialize(state) {
        ...
        var fields = ko.observableArray((state.fields || []).map((value, index, array) => { return value.displayName; }));
    ...
    commit() {
        var groups = this.groups.items.map((item) => {
            return item.fields();
        });
        var summaryColumns = [];
        if (!this.groups.isEmpty) {
            this.fields.items.forEach((fieldName) => {
                var field = getFirstItemByPropertyValue(this.initialFields, 'displayName', fieldName);
