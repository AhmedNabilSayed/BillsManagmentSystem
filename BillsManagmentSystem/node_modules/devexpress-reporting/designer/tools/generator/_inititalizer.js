...
import { ExpressionSmartTag } from '../smartTags/expressionSmartTag';
...
export class ReportDesignerInitializer extends CommonDesignerGenerator {
    constructor(options) {
        ...
        var navigation = new NavigateByReports({
        ...
        var undoEngine = ko.computed(() => { return navigation.currentTab() && navigation.currentTab().undoEngine; });
        ...
        var surface = ko.computed(() => this._reportcontext() && this._reportcontext().surface);
        var canAddItems = ko.computed(() => model() && model().language() === defaultCulture);
        ...
        var isDefaultLanguage = ko.computed(() => {
    ...
    subscribeIncomeReport(report, reportUrl, dataSourceRefs, knownEnums) {
        this._addDisposable(report.subscribe((newValue) => {
            var newModel = new ReportViewModel(newValue, undefined, knownEnums);
    ...
    _tryAddScriptEditor(isScriptsDisabled) {
        ...
        if (!isScriptsDisabled) {
            var scriptsEditor = new ScriptsEditor(this.buildingModel.model, this.buildingModel.controlsHelper.allControls);
            this._addDisposable(scriptsEditor.editorVisible.subscribe((newValue) => {
                if (newValue) {
                    var focusedControl = this.buildingModel.selection.focused();
                    ...
                    var resizeFunction = () => setTimeout(() => {
                        scriptsEditor.editorContainer().resize();
                    }, 1);
                    if (!scriptsEditor.editorContainer())
                        var innerSubscription = scriptsEditor.editorContainer.subscribe((newVal) => {
            ...
            var controlTypes = Object.keys(controlsFactory().controlsMap);
            for (var index = 0; index < controlTypes.length; index++) {
                ['scripts', 'scriptReferencesString', 'scriptLanguage'].forEach((propertyDisplayName) => {
                    var propertyInfo = controlsFactory().getPropertyInfo(controlTypes[index], propertyDisplayName);
    ...
    _getControls(controls, filter, isNoneItemAdded = true) {
        return ko.pureComputed(() => {
            ...
            var allControls = result.map((item) => {
    ...
    _createEmptyReportItemsProvider() {
        ...
        var parameters = new ParametersViewModel(reportModel);
        var dataSourceHelper = new DataSourceHelper(reportModel.objectStorage, reportModel.dataSourceRefs, undefined);
        reportModel.dataSourceHelper(dataSourceHelper);
        var fieldListDataSourceHelper = new FieldListDataSourcesHelper();
        fieldListDataSourceHelper.updateDataSources(dataSourceHelper, reportModel, parameters.parameters);
        var wrappedCallback = fieldListDataSourceHelper.wrapFieldsCallback(ReportDataSourceService.fieldListCallback, () => undefined);
    ...
    addErrorPanelViewModel(element, options) {
        this.addElement('errorPanelViewModel', () => {
            ...
            var $root = $.fn.constructor(element);
            var $window = $.fn.constructor(window);
            var $progress = $root.find('.dxrd-error-panel');
            var updateProgressBarPosition = getDockedElementCallback($progress, $root, $window, '.dxrd-error-panel', { bottom: true });
            this._addDisposable(errorPanel.collapsed.subscribe(() => {
                updateProgressBarPosition(element);
            }));
            var wrapped = () => updateProgressBarPosition(element);
    ...
    addPreview(options) {
        ...
        createPreview({
            ...
            this.addElement('reportPreviewModel', () => {
                this._addDisposable(reportPreviewModel.reportPreview.previewVisible.subscribe((newValue) => {
                    this.buildingModel.designMode(!newValue);
    ...
    initializeCalculatedFieldsSource() {
        var calculatedFieldsSource = ko.computed(() => this._reportcontext() && this._reportcontext().calcFieldsSource);
    ...
    initializeFieldListItemsExtenders() {
        var fieldListItemsExtenders = ko.computed(() => this._reportcontext() && this._reportcontext().fieldListItemsExtenders);
        this.addElement('fieldListItemsExtenders', () => fieldListItemsExtenders);
        return this;
    }
    initializeParameters() {
        var parameters = ko.computed(() => this._reportcontext() && this._reportcontext().parameters);
        this.addElement('parameters', () => parameters);
        this._parameters = parameters;
        return this;
    }
    initializeFieldListProvider() {
        var fieldListProvider = ko.computed(() => this._reportcontext() && this._reportcontext().fieldListProvider);
        this.addElement('fieldListProvider', () => fieldListProvider);
        return this;
    }
    initializeReportItemsProvider() {
        var reportItemsProvider = ko.computed(() => this._reportcontext() && this._reportcontext().reportItemsProvider);
        this.addElement('reportItemsProvider', () => reportItemsProvider);
        return this;
    }
    initializeDataBindingsProvider() {
        var dataBindingsProvider = ko.computed(() => this._reportcontext() && this._reportcontext().dataBindingsProvider);
        this.addElement('dataBindingsProvider', () => dataBindingsProvider);
        return this;
    }
    initializeDisplayNameProvider() {
        var displayNameProvider = ko.computed(() => this._reportcontext() && this._reportcontext().displayNameProvider);
    ...
    initializeExpressionDisplayNameProvider() {
        var expressionDisplayNameProvider = ko.computed(() => this._reportcontext() && this._reportcontext().expressionDisplayNameProvider);
        this.addElement('expressionDisplayNameProvider', () => expressionDisplayNameProvider);
        return this;
    }
    initializeDataSourceHelper() {
        var dataSourceHelper = ko.computed(() => this._reportcontext() && this._reportcontext().dataSourceHelper);
        this.addElement('dataSourceHelper', () => dataSourceHelper);
        var fieldListDataSourceHelper = ko.computed(() => this._reportcontext() && this._reportcontext().fieldListDataSourceHelper);
    ...
    addSelection(func = (settings) => {
        ...
        var rtl = ko.computed(() => { return this.buildingModel.surface() && this.buildingModel.surface().rtl(); });
        var snapLinesCollector = new ReportSnapLinesCollector(rtl);
    ...
    createControlProperties(editors, groups, accessibilityProvider) {
        var properties = super.createControlProperties(editors, groups, accessibilityProvider);
        properties.createEditorAddOn = (editor) => {
            var editorAddOn = new ValueEditorAddOn(editor, properties.popupService);
            var editorExression = new ExpressionEditorAddOn(editor, properties.popupService, 'dxrd-svg-properties-propertyexpression');
    ...
    addMenu(func = (settings) => void 0) {
        var settings = new ReportMenuSettings();
        func(settings);
    ...
    addControlsHelper(func = (settings) => {
        var helper = ko.computed(() => this._reportcontext() && this._reportcontext().controlsHelper);
        this._addDisposable(helper);
        var controlHelper = new ReportDesignerControlsHelper(helper);
        settings
    ...
    addSmartTagModel() {
        var smartTagFactory = Object.assign({}, UtilsSmartTagFactory());
        ...
        var smartTagHelper = new SmartTagModel(this._selection, this.buildingModel.surface, this.buildingModel['verticalScrollOffset'], smartTagFactory);
    ...
    _createPropertiesTab() {
        return new TabInfo({
            ...
            visible: ko.pureComputed(() => { return !!this.buildingModel.model(); }),
    ...
    _createExpressionsTab(context) {
        var expressionGridModel = ko.computed(() => {
            var editableObject = this.buildingModel.editableObject();
        ...
        var expressionTab = new TabInfo({
            ...
            visible: ko.pureComputed(() => { return this.buildingModel.model() && this.buildingModel.model()._dataBindingMode() !== DataBindingMode.Bindings; }),
            disabled: ko.pureComputed(() => {
                return !expressionGridModel();
            })
        });
        var currentFilter = undefined;
        this._addDisposable(expressionTab.active.subscribe((newVal) => {
    ...
    _createReportExplorerTab() {
        var _a;
        var reportExplorer = new ReportExplorerModel(this.buildingModel.model, this.buildingModel.editableObject, (targetObject) => {
            var propertiesTab = this.buildingModel.tabPanel.getTabByName('Properties');
    ...
    _createFieldListTab(designerCallbacks, startDataSourceWizard) {
        ...
        var treeListOptions = {
            ...
            factory: new FieldListItemFactory(ko.observable(false)),
    ...
    addTabPanel(panel, addTabInfo = () => {
        ...
        var contextSubscription = this._reportcontext.subscribe((newVal) => {
            if (!!newVal) {
    ...
    addShowPreviewMethod() {
        this.addElement('showPreview', () => () => {
            var reportPreview = this.buildingModel.reportPreviewModel.reportPreview;
            reportPreview.previewVisible(true);
            if (!this.buildingModel.model()) {
                var subscription = this.buildingModel.model.subscribe((newVal) => {
    ...
    initializeUIEffects(applyBindings, element) {
        this._addDisposable(this.buildingModel.editableObject.subscribe(() => {
            var $scrollView = $.fn.constructor('.dxrd-designer .dxrd-properties-wrapper').find('.dx-scrollview');
        ...
        var updateSurfaceContentSize__ = updateSurfaceContentSize(this.buildingModel.surfaceSize, element, this.rtl);
        var updateSurfaceContentSizeLocalizationMode__ = updateSurfaceContentSizeLocalizationMode(this.buildingModel.surfaceSize, element, this.rtl);
        var updateSurfaceContentSize_ = () => {
            ...
            var $root = $.fn.constructor(element).find('.dxrd-designer').eq(0);
        ...
        var updateSurfaceContentSize_async = () => {
        ...
        this.buildingModel.onViewPortScroll = (viewPort) => {
            var $viewPort = $.fn.constructor(viewPort), $surface = $viewPort.closest('.dxrd-surface');
        ...
        this._addDisposable(this.buildingModel.tabPanel.width.subscribe(() => {
            updateSurfaceContentSize_async();
        }));
    ...
    _createNewReportFromWizardState(reportWizard, designerModel, wizardState, state, deferred) {
        ReportWizardService.generateReportFromWizardState(wizardState, reportWizard._requestModelType, state(), (wizardModel) => {
            ...
            .done((result) => {
            var modelJson = JSON.parse(result.reportModel);
            var newReport = new ReportViewModel(modelJson, undefined, result.knownEnums);
        ...
        var start = () => {
            ...
            reportWizard.start((reportWizardState) => {
                ...
                if (reportWizardState.newDataSource) {
                    var ds = _restoreDataSourceFromState(reportWizardState.newDataSource);
                ...
                var createReport = (wizardState) => {
                    designerModel.isLoading(true);
                    designerModel.navigateByReports.currentTab().undoEngine.start();
                    designerModel.isDirty(true);
                    var _patchedInfo = this._patchReportBeforeRedesign(designerModel.model(), wizardState);
                    ReportWizardService.generateReportFromWizardState(wizardState, reportWizard._requestModelType, state(), (wizardModel) => {
                        reportWizard.events.call('beforeFinish', { state: wizardState, wizardModel: wizardModel });
                    }, _patchedInfo)
                        .done((result) => {
    ...
    _createMultiQueryDataSourceWizardAction(multiQueryDataSourceWizard, designerModel) {
        var isDisabled = ko.pureComputed(() => { return !designerModel.model(); });
        var isVisible = ko.pureComputed(() => multiQueryDataSourceWizard.canRunWizard());
        var menuAction = {
        ...
        return ((actions) => {
            ...
            if (reportStorageWebIsRegister()) {
                actions.push({
                    ...
                    clickAction: () => {
                        designerModel.isLoading(true);
                        var wizardState = createReportWizardState();
            ...
            actions.push({
                ...
                text: 'Full Screen',
                displayText: () => getLocalization('Full Screen', 'ASPxReportsStringId.ToolBarItemText_FullScreen'),
                imageClassName: 'dxrd-image-fullscreen',
                imageTemplateName: () => designerModel.fullScreen() ? 'dxrd-svg-toolbar-fullscreen-exit' : 'dxrd-svg-toolbar-fullscreen',
                disabled: ko.observable(false),
            ...
            this._disposables.push(designerModel.fullScreen);
            if (scriptsEditor) {
                actions.push({
                    id: ActionId.Scripts,
                    text: 'Scripts',
            ...
            if (reportPreview) {
                var previewAction = {
                    ...
                    disabled: ko.pureComputed(() => { return !report(); }),
                    visible: ko.pureComputed(() => { return !reportPreview.previewVisible(); }),
    ...
    _patchReportBeforeRedesign(reportViewModel, state) {
        var _a;
        var useInitialDataSource = false;
        if (!state.newDataSource && state.dataSource) {
            var stateDsInfo = _restoreDataSourceFromState(state.dataSource);
        ...
        var reportJson = reportViewModel.serialize();
    ...
    addContextActions(func = (settings) => {
        var isDisabled = () => this.buildingModel.model() && this.buildingModel.model().language() !== defaultCulture;
        var disposableProviders = [
            ...
            new ReportActions(this._designerCallbacks.componentAdded),
    ...
    addActionList(actionListFunc = () => {
        var designerShortcutsEnabled = ko.computed(() => {
            var isDesignMode = this.buildingModel.designMode();
            var isWizardVisible = this.buildingModel.wizard && this.buildingModel.wizard.isVisible();
            var isDataSourceWizardVisible = this.buildingModel.dataSourceWizard && this.buildingModel.dataSourceWizard.isVisible();
            var isMultiQueryDataSourceWizardVisible = this.buildingModel.multiQueryDataSourceWizard && this.buildingModel.multiQueryDataSourceWizard.isVisible();
            var isLocalizationModeVisible = this.buildingModel.localizationMode && this.buildingModel.localizationMode();
        ...
        var actions = new ActionLists(this.buildingModel.surface, this.buildingModel.selection, this.buildingModel.undoEngine, this._customizeDesignerActions(this.buildingModel, this._designerCallbacks.customizeActions, this._designerCallbacks.exitDesigner, () => this._reportcontext() && this._reportcontext().state() || this.buildingModel.state, this._designerCallbacks), designerShortcutsEnabled, reportCopyPasteStrategy(this._designerCallbacks.componentAdded), this.buildingModel.zoomStep, _ => !this.buildingModel.canAddItems());
    ...
    _createChartDesignerPart(context) {
        var chartValueBindingProvider = ko.computed(() => context() && context().chartValueBindingProvider);
        this._addDisposable(chartValueBindingProvider);
        var chartDesignerOptions = createChartDesignerOptions(this.buildingModel, this.buildingModel.dataSourceHelper, this.buildingModel.model, this.buildingModel.parameters, chartValueBindingProvider);
        this.addElement('runChartDesigner', () => (chart) => {
            chartDesignerOptions.run(chart);
        });
        this.addElement('chartDataSources', () => ko.computed(() => {
            var pivotGrids = this.buildingModel.controlsHelper.allControls().filter(item => { return item instanceof XRPivotGridViewModel; }).map(item => { return { displayName: item.name, value: item }; });
            var usedDataSources = this.buildingModel.dataSourceHelper() && this.buildingModel.dataSourceHelper().usedDataSources() && this.buildingModel.dataSourceHelper().usedDataSources().map(item => { return { displayName: item.name, value: item.data }; }), result = [].concat(pivotGrids, usedDataSources);
            return result;
        }));
    ...
    addParts(func = (parts) => {
        var reportConverter = new ReportConverter(this.buildingModel.controlsHelper, this.buildingModel.undoEngine, this._dataBiningMode);
        ...
        [DesignerBaseElements.MenuButton, DesignerBaseElements.Toolbox].forEach((item) => {
            var oldItem = parts.filter(part => part.id === item)[0];
    ...
    addDefaultAddons(addons = [
        { id: ReportDesignerAddOns.Preview, templateName: ReportDesignerAddOns.Preview, model: this.buildingModel.reportPreviewModel },
    ...
    tryAddInlineRichTextEdit() {
        var inlineRichEdit = registerRichEditInline()(this._selection);
        if (inlineRichEdit) {
    ...
    addPopularProperties(controlsFactory, accessibilityProvider) {
        ...
        this.addElement('popularVisible', () => ko.pureComputed(() => {
            return this.buildingModel.popularProperties._editors().some((x) => { return x.visible(); }) ||
    ...
    addWizards(connectionStrings, wizardSettings, cusomizeSettingsFunc) {
        var usedDataSources = ko.pureComputed(() => {
            var dataSources = this.buildingModel.dataSourceHelper() && this.buildingModel.dataSourceHelper().usedDataSources();
            return dataSources && dataSources.filter(x => !!x.data);
        });
        var predefinedDataSources = ko.pureComputed(() => { var _a; return ((_a = this.buildingModel.dataSourceHelper()) === null || _a === void 0 ? void 0 : _a.availableDataSources) || []; });
        this._disposables.push(usedDataSources, predefinedDataSources);
        var initializerSettings = new WizardsInitializerSettings(connectionStrings, wizardSettings, this._designerCallbacks, this.rtl, usedDataSources, predefinedDataSources);
