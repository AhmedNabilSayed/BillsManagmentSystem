    ...
    generateOptions(dataSources, popupContainer) {
        if (!this.options) {
            var disabled = ko.computed(() => {
                return this.disabled() || !dataSources() || dataSources().length === 0;
            });
            var value = ko.computed({
                read: () => {
                    var unwrappedDataSources = dataSources();
                    var dataSource = unwrappedDataSources.filter(x => !!x && (x.value === this.value()))[0];
                    return ko.unwrap(dataSource && dataSource.displayName);
                },
                write: (newVal) => {
                    var unwrappedDataSources = dataSources();
                    var dataSource = unwrappedDataSources.filter(x => !!x && (ko.unwrap(x.displayName) === newVal))[0];
