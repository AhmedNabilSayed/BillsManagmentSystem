...
export class SeriesPointModel extends SerializableModel {
    constructor(model, series, serializer) {
        ...
        this.argumentSerializableInfo = ko.computed(() => {
            ...
            var editor = editorTemplates.getEditor('text');
        ...
        this.getInfo = () => {
            var dataMember = this.series && this.series.valueDataMembers();
            if (!dataMember)
                return seriesPointSerializationsInfo;
            var valueDataMemberInfo = dataMember.getInfo().map(info => $.extend({}, info, { editor: this.isDateType ? editorTemplates.getEditor('date') : editorTemplates.getEditor('numeric') }));
            var info = $.extend(true, [], seriesPointSerializationsInfo);
...
export var valuesSerializable = { propertyName: 'valuesSerializable', modelName: '@ValuesSerializable', from: (val) => { return ko.observable(val); }, toJsonObject: SeriesPointModel.valueToJsonObject }, argumentSerializable = { propertyName: 'argumentSerializable', modelName: '@ArgumentSerializable', displayName: 'Argument', localizationId: 'DevExpress.XtraCharts.SeriesPoint.Argument', editor: editorTemplates.getEditor('text') }, colorSerializable = { propertyName: 'colorSerializable', modelName: '@ColorSerializable', displayName: 'Color', localizationId: 'DevExpress.XtraCharts.SeriesPoint.Color', from: colorFromString, toJsonObject: colorToString, editor: editorTemplates.getEditor('customColorEditor') };
export var seriesPointSerializationsInfo = [argumentSerializable, valuesSerializable, colorSerializable];
